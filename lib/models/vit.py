...
class VisionTransformer(nn.Module):
    """ Vision Transformer """
    def __init__(self, img_size=[224], patch_size=16, use_avg_pooling_and_fc=True, in_chans=3, num_classes=0, embed_dim=768, depth=12,
...
def vit_small(patch_size=16, use_avg_pooling_and_fc=True, num_classes=128, pretrained=True, **kwargs):
    model = VisionTransformer(
        patch_size=patch_size, use_avg_pooling_and_fc=use_avg_pooling_and_fc, num_classes=num_classes, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4,
...
def vit_base(patch_size=16, use_avg_pooling_and_fc=True, num_classes=128, pretrained=True, **kwargs):
    model = VisionTransformer(
        patch_size=patch_size, use_avg_pooling_and_fc=use_avg_pooling_and_fc, num_classes=num_classes, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4,
        qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    if pretrained:
        model.load_state_dict(torch.hub.load('facebookresearch/dino:main', 'dino_vitb16').state_dict(), strict=False)
    return model
